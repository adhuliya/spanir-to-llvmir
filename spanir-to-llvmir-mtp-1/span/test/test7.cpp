...
int main()
{
  ...
  expr::VarExpr varex_x("main:x", &ty_var_x); // expr for varexpr x (int x = a+b);

  expr::BinaryExpr binaryexpr(&varex_a, BO_ADD_OC, &varex_b);    // expr for binaryexpr (a+b);

  instr::AssignI assigni_x(&varex_x, &binaryexpr);  // for assign instr int x = a+b;

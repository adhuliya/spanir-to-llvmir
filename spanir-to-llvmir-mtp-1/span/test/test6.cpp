...
int main()
{
  ...
  types::Type ty_var2(types::TY_PTR);     // used for local variable p of main
  
  types::Type ty_expr1_1(types::TY_INT32);  // used for x in expr 'x+y'
  expr::VarExpr varex1("x", &ty_expr1_1);          // used for varax x
  
  types::Type ty_expr1_2(types::TY_UINT32); 
  types::PointerType pointertype(&ty_expr1_2, 1); 
  expr::UnitExpr unitexpr(&pointertype, EX_UNARY_EXPR);   // used for pointer *p
  expr::UnaryExpr unaryexpr("main:p", UO_DEREF_OC, &unitexpr);

  instr::AssignI assigni(&varex1, &unaryexpr); // used for AssignI
  ...
  tunit::TUnit currTUnit(
    ...
    "span.tests.test1",
    //desc::
    "A simple sequential program. 1. int x=*p; ",
    //VARS::
    {
